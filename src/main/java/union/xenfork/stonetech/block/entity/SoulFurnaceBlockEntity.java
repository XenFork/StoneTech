...
import union.xenfork.stonetech.recipe.SoulSmeltingRecipe;
import union.xenfork.stonetech.screen.SoulFurnaceScreenHandler;

import java.util.Optional;

import static union.xenfork.stonetech.block.entity.ModBlockEntities.SOUL_FURNACE_BLOCK_ENTITY;

public class SoulFurnaceBlockEntity extends BlockEntity implements ExtendedScreenHandlerFactory, ImplementedInventory {
    ...
    public SoulFurnaceBlockEntity(BlockPos pos, BlockState state) {
        super(SOUL_FURNACE_BLOCK_ENTITY, pos, state);
    ...
    private void craftItem() {
        Optional<RecipeEntry<SoulSmeltingRecipe>> recipe = getCurrentRecipe();
    ...
    private Optional<RecipeEntry<SoulSmeltingRecipe>> getCurrentRecipe() {
        SimpleInventory inv = new SimpleInventory(this.size());
        for(int i = 0; i < this.size(); i++) {
            inv.setStack(i, this.getStack(i));
        }

        return getWorld().getRecipeManager().getFirstMatch(SoulSmeltingRecipe.Type.INSTANCE, inv, getWorld());
    }
